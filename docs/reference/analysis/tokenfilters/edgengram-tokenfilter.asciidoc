...
The `edge_ngram` token filter emits
...
With the default settings, the `edge_ngram` token filter produces N-grams with
minimum length `1` and maximum length `2`:
...
The `edge_ngram` token filter accepts the following parameters:
